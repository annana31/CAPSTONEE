...
export default function Login({ onLogin, onStudentAccess }) {
  ...
  // ── BACKEND: Authenticate staff/admin against tbl_staff ──
  const handleLogin = async (inputUsername, inputPassword, expectedRole) => {
    setError("");
    setLoading(true);

    try {
      // Query tbl_staff for matching username
      const { data, error: dbError } = await supabase
        .from("tbl_staff")
        .select("staff_id, user_role, username, password")
      ...
      // Check password (plain-text comparison as per your schema)
      ...
      // If logging in via Admin panel, enforce role check
      ...
      // Success — pass the username (or role) up to parent
      onLogin(data.username, data.user_role);
    } catch (err) {
      ...
      <div className="login-page">
        ...
        <div className="login-center">
          <div className="login-card">
            ...
            <button
              onClick={() => handleLogin(adminUsername, adminPassword, "admin")}
              className="login-btn"
              disabled={loading}
            >
    ...
    <div className="login-page">
      ...
      <div className="login-center">
        <div className="login-card">
          ...
          <button
            onClick={() => handleLogin(username, password, null)}
            className="login-btn"
            disabled={loading}
          >
